...
def isLinkToTrope(href):
    return (href 
            and re.compile("/pmwiki/pmwiki.php").search(href) 
            and not re.compile("/Main/").search(href)
            and not re.compile("/Franchise/").search(href)
            )
...
def grab(pageName: str):
    ...
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, "lxml")
        for link in soup.find(id="main-article").find_all(href=isLinkToTrope):
            print(link["href"])
            print(link.string.strip())

print(grab("AbileneParadox"))
'''
go to tvtropes
